...
import { Star, MapPin, Users, Bed, Bath, Wifi, Wind, Flame, Waves, Coffee, Utensils, Share2, Heart, ChevronLeft, ChevronRight } from 'lucide-react';
        ...
        <div className="max-w-7xl mx-auto px-4 py-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 h-[300px] md:h-[500px] mb-12">
                <div className="md:col-span-3 relative rounded-3xl overflow-hidden group">
                    ...
                    />
                    <button
                        onClick={() => setMainImage((prev) => (prev > 0 ? prev - 1 : listing.images.length - 1))}
                        className="absolute left-4 top-1/2 -translate-y-1/2 p-2 bg-white/80 dark:bg-black/40 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                        <ChevronLeft />
                    </button>
                    <button
                        onClick={() => setMainImage((prev) => (prev < listing.images.length - 1 ? prev + 1 : 0))}
                        className="absolute right-4 top-1/2 -translate-y-1/2 p-2 bg-white/80 dark:bg-black/40 rounded-full opacity-0 group-hover:opacity-100 transition-opacity"
                    >
                        <ChevronRight />
                    </button>
                </div>
                <div className="hidden md:grid grid-rows-3 gap-4">
                    {listing.images.slice(0, 3).map((img, i) => (
                        <div
                            key={i}
                            className={`rounded-2xl overflow-hidden cursor-pointer border-4 transition-all ${mainImage === i ? 'border-primary-600' : 'border-transparent'}`}
